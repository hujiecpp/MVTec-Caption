...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # TODO: jie - check if clip text trainable
    ...
    def get_learned_conditioning(self, c, cond_text, x=None,name=None):
        position=None
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                c,position = self.cond_stage_model.encode(c, cond_img=x, cond_text=cond_text,embedding_manager=self.embedding_manager,name=name)
        ...
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode: #不执行
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c,position = self.get_learned_conditioning(xc,cond_text=total_dict['text'],x=masked_img,name=name)
                    total_dict['position']=position
                else:
                    c,position = self.get_learned_conditioning(xc.to(self.device),cond_text=total_dict['text'],x=masked_img,name=name)
            else: #执行这条
                c = xc
    ...
    def forward(self, x, c, *args,mask=None,masked_img=None, name=None,**kwargs):
        ...
        if self.model.conditioning_key is not None: #执行
            ...
            if self.cond_stage_trainable:#执行
                c,_ = self.get_learned_conditioning(c,cond_text=kwargs['text'],x=masked_img,name=name)
    ...
    def p_losses(self, x_start, cond, t,mask=None, noise=None, text=None):
        noise = default(noise, lambda: torch.randn_like(x_start))
        if mask is not None:
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, unconditional_only=False,ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log["prompt_text"] = log_txt_as_img((x.shape[2], x.shape[3]), total_dict["text"])
        ...
        if sample:
            ...
            uc,_ = self.get_learned_conditioning(len(c) * [""],cond_text=total_dict["text"], name=total_dict["name"])
